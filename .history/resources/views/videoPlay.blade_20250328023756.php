...
<body>
    ...
    <script>
        ...
        if (Hls.isSupported()) {
            ...
            // التأكد من أن المشغل جاهز قبل إرفاق HLS
            player.ready(function () {
                hls.attachMedia(player.el()); // الحل الصحيح بدلاً من player.tech().el()
            });

            hls.on(Hls.Events.MANIFEST_PARSED, function (event, data) {
                console.log("MANIFEST_PARSED:", data.levels);

                // تفعيل اختيار الجودة داخل مشغل الفيديو
                player.hlsQualitySelector({
                    displayCurrentQuality: true, // عرض الجودة الحالية
                    defaultQuality: "auto" // تحديد الجودة الافتراضية
                });
            });
